...
$contextPath = 'context/context.json';
$integrationsPath = 'context/integrations.json';
$protectedPage = false;
$password = "go";

// Handle login (keeping same structure as oleg-browser)
$loginError = '';
...
<head>
    ...
    <title>n8n Partners - Workflow Templates</title>
...
<?php if ($protectedPage && !$isLoggedIn): ?>
    <!-- Login Form (keeping same as oleg-browser) -->
        ...
        <div class="max-w-md w-full">
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6 text-center">
                    ...
                    <p class="mt-2 text-purple-100">Access n8n Partners Workflow Library</p>
                </div>
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="" class="space-y-4">
                        ...
                        <button 
                            type="submit" 
                            class="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white py-3 px-4 rounded-lg font-medium hover:from-purple-700 hover:to-blue-700 transition-all duration-200 transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2"
                        >
...
<?php else: ?>
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="bg-gray-50 px-6 py-4 border-b">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600" id="total-integrations">-</div>
                        <div class="text-sm text-gray-600">Integrations</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600" id="total-categories">-</div>
            ...
            <div class="p-6 border-b bg-white">
                <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="flex gap-2">
                        <select id="integration-filter" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
    ...
    <script>
        ...
        async function loadData() {
            try {
                // Load context and integrations data
                const [contextResponse, integrationsResponse] = await Promise.all([
                    fetch('context/context.json'),
                    fetch('context/integrations.json')
                ]);

                const contextData = await contextResponse.json();
                const integrations = await integrationsResponse.json();

                // Create integrations lookup
                integrations.forEach(integration => {
                    integrationsData[integration.integration] = integration;
                });

                // Process workflows data
                workflowsData = Object.keys(contextData).map(key => {
                    const workflow = contextData[key];
                    const integration = (workflow.integration || '').trim();
                    const integrationInfo = integrationsData[integration] || {};
                    
                    return {
                        name: key,
                        path: workflow.path,
                        integration: integration,
                        category: integrationInfo.category || 'Uncategorized',
                        description: integrationInfo.description || '',
                        searchText: (key + ' ' + integration + ' ' + (integrationInfo.category || '') + ' ' + (integrationInfo.description || '')).toLowerCase()
                    };
                });
        ...
        function renderWorkflows(filteredData = workflowsData) {
            ...
            const cards = filteredData.map(workflow => {
                const hasJson = checkJsonExists(workflow.path);
                ...
                return `
                    <div class="workflow-card bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer border hover:border-purple-200"
                         onclick="viewWorkflow('${workflow.path}', '${escapeHtml(workflow.name)}')"
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex items-center">
                                    <i class="${integrationIcon} text-purple-600 mr-2"></i>
                                ...
                                <div class="flex items-center">
                                    ${hasJson ? 
                                        ...
                                        '<i class="fas fa-times text-gray-400 text-sm" title="JSON Not Available"></i>'
                                    }
                                </div>
                            </div>

                            <!-- Workflow Name -->
                            <h3 class="text-lg font-semibold text-gray-900 mb-3 line-clamp-2 hover:text-purple-600 transition-colors">
                                ${workflow.name}
                            </h3>
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                ...
                                ${hasJson ? 
                                    ...
                                    '<span class="text-xs text-gray-400">No JSON</span>'
                                }
                            </div>
                        </div>
                    </div>
        ...
        function checkJsonExists(path) {
            // For now, assume all workflows have JSON available
            // In a real implementation, you'd check if the file exists
            return path && path.includes('.json');
        }
        ...
        function updateStats() {
            ...
            const availableDownloads = workflowsData.filter(w => checkJsonExists(w.path)).length;
        ...
        async function viewWorkflow(path, name) {
